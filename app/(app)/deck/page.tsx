...
import { shouldAutoGenerate } from "@/lib/utils/deckGenerate";
...
function DeckContent() {
  ...
  useEffect(() => {
    if (
      shouldAutoGenerate({
        generateFlag: searchParams.get("generate"),
        alreadyTriggered: autoGenerated.current,
        generating,
      })
    ) {
      autoGenerated.current = true;
      // Strip the one-shot `?generate=true` flag from the URL so a page reload
      // (which re-mounts this component with a fresh guard) can't kick off a
      // second, wasteful generation of 20 more cards.
      if (typeof window !== "undefined") {
        window.history.replaceState(null, "", "/deck");
      }
      generateBatch(1).catch((e) =>
